...
function Afectado_Terceros_screen({navigation: {navigate}}) {
  ...
  const sigPressed = async () => {
    if (
      ...
      userDefault.DatosAfectado.NombreA = nombrea.replace('\n', '');
  ...
  return (
    <View style={styles.container_}>
      <ScrollView style={styles.container_scr}>
        ...
        <View style={styles.container_diag}>
          <Text style={styles.tx_title}>
            Datos De Identificacion Del Agraviado
          </Text>

          <Text style={styles.diaf_tx_corr}>Nombre Del Agravido</Text>
          <TextInput
            style={styles.diag_txin_nmb}
            ref={ref_txin_ag_nmb}
            value={nombrea}
            onChangeText={text => setNombrea(text)}
            onSubmitEditing={() => ref_txin_corr.current.focus()}
...
const styles = StyleSheet.create({
  ...
  //#region Datos De Identificacion Del Agraviado Styles
  container_diag: {
    flex: 1,
  },
  diag_txin_nmb: {
...
export default Afectado_Terceros_screen;
